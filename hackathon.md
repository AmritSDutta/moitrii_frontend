...
8. **AgentMail Sender Alignment & Scaling Roadmap:** Configured the companion sender address to `moitrii@agentmail.to` in `convex/agents.ts` to match AgentMail's free-tier single custom inbox constraint, while documenting the production roadmap to provision 1 dedicated virtual AgentMail address per user upon scaling.
9. **Testing & Build Verification:** 63 / 63 tests passing across all 12 Vitest suites (`npm test`), and 0 TypeScript compilation errors in production SPA build (`npm run build`). Convex features: workflows, components, schema, compound indexes, full-text search, queries, mutations, actions, crons, file storage (`convex/schema.ts`, `convex/agents.ts`, `convex/content.ts`, `convex/subscribers.ts`, `convex/emails/brevo.ts`, `convex/crons.ts`, `src/lib/youtube.ts`, `src/pages/ContentReaderPage.tsx`, `src/pages/HomePage.tsx`, `src/pages/UnsubscribePage.tsx`, `src/App.tsx`, `docs/**`, `README.md`).
